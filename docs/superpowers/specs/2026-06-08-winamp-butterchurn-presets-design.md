...
- 维护一张**固定的确定性映射表**(JSON,随脚本提交):`{ "<two-word>": "<原 preset key>" }`,恰好 100 条。
- 半自动流程:脚本提取候选 + 去重,人工逐个赋予描述性两词名(基于原名语义/preset 视觉倾向)。最终表是确定的、可重跑、不随机。
- 全部唯一,全小写短横线,匹配 `^[a-z]+-[a-z]+$`,与现有 label 风格一致(如 `hex-wave`)。
...
2. **命名映射校验单测**:断言映射表 100 条、label 全唯一且匹配 `^[a-z]+-[a-z]+$`、每个 presetKey 存在于真实包。
